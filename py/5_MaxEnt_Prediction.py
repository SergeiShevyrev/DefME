...
filename_model=os.path.join(filedir,model_maxent_name);
fileext='tif';
pathrowfolder="111_028"
datefolder="2019_10_16"

fileoutname="predicted_value_on_saved_model.tif";
...
file_model_data_name_path_out=os.path.join(dir_products_path,(file_model_data_name));
...
for col_name in [*model_data_df]:
    if col_name.lower().startswith('dpc'):
        ...
        """
        if col_name == 'DPC1_epidote' or col_name == 'DPC2_goetite' \
                or col_name == 'DPC2_quartz' or col_name == 'DPC2_alunite'\
                or col_name == 'DPC2_illite' or col_name == 'DPC2_chlorite':
                dpc1_columns.append(col_name);
        """
...
#
#ПРИМЕНЕНИЕ МОДЕЛИ К 100% СТРОК И ВЫВОД 
...
pred = clf.decision_function(all_data_std) #прогноз значений распределения
...
#в примере нормальны!!! отрицательные значения, рассматриваем нормализованые значения
pred_norm=(pred-pred.min())/(pred.max()-pred.min());    
    
    
"""    
dataout_all_predicted=regr.predict(all_data_np_array[:,3:])

dataout_all_predicted=(dataout_all_predicted-np.min(dataout_all_predicted))\
                /(np.max(dataout_all_predicted)-np.min(dataout_all_predicted));
"""
...
saveGeoTiff(predicted_raster,os.path.join(dir_products_path,\
    "predicted_{}_".format('')+pathrowfolder+"_"+datefolder+".tif"),\
    gdal_object,0,0);
...
fig.suptitle('Response curves for testing dataset')
