...
def Datasort4(data_table,target_variable_key):
    ts=0.8;
    column_names=list(data_table);   
    #picking up data
    id=[];      #identifiers array
    ...
    for idx in column_names:
        if idx.lower().find('id')!=-1:
            id=list(data_table[idx]);
        if idx.find(target_variable_key)!=-1:
            oredist=list(data_table[idx]);
        #gather parameters for correlation

        if idx.lower().find('ioa')!=-1 or \
           idx.lower().find('cm')!=-1 or\
           idx.lower().find('pca')!=-1:
               params.update({idx:list(data_table[idx])});
...
def draw_circle(img,r,c,rad):
    ...
    #split data into learning and testing sets разбиение данных на выборку для обучения (30%) и прогноза
    datain_id1, datain_id2, dataout1, dataout2=train_test_split(datain_and_id,dataout,random_state=None,train_size=ts);

    return datain_id1, datain_id2, dataout1, dataout2,param_names,params;
